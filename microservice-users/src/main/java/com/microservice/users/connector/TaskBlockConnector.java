...
@Component
public class TaskBlockConnector {
    private final String HOST = "api-tasks";
    private final String ENDPOINT = "get-taskblocks";
    private HttpConnectorConfig configuration;

    @Autowired
    public TaskBlockConnector(HttpConnectorConfig configuration) {
        this.configuration = configuration;
    }

    @CircuitBreaker(name = "api-tasks", fallbackMethod = "fallbackGetTaskBlocks")
    public List<TaskBlockDTO> getTaskBlocks(Long userId) {
        ...
        return client.get()
                ...
                .retrieve()
                .bodyToMono(new ParameterizedTypeReference<List<TaskBlockDTO>>() {})
                .share()
                .block();

    }

    public List<TaskBlockDTO> fallbackGetTaskBlocks(Long userId, CallNotPermittedException ex) {
        System.out.println("calling to fallbackGetUser-1");

        return List.of();
    }

    public List<TaskBlockDTO> fallbackGetTaskBlocks(Long userId, Exception ex) {
